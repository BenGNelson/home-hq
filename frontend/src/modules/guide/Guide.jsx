...
const ENDPOINTS = [
  ...
  ['/api/solar', 'Live Enphase solar — production, consumption/net, plus battery SoC/charge, the 4-node power flow, and self-sufficiency %, read from the Envoy’s local API via pyenphase.'],
  ['/api/solar/history', 'Intraday solar production/consumption/battery-SoC trend (in-app sampler → SQLite) — powers the Solar page’s day curves.'],
  ['/api/solar/panels', 'Per-microinverter output (indexed, no serials) — powers the Solar page’s per-panel array map.'],
  ...
  ['/api/library · /{section} · /file · /games/cover · /books/cover · /papers/cover · /comics/page · /listen-progress · /reading-progress · /continue', 'The owned-content hub: sections + counts + a few cover previews (games, books, comics, audiobooks, magazines & papers), a section’s items, a range-capable traversal-guarded file stream (used by the emulator, the PDF + ebook readers, and the audiobook player — audio gets a real MIME type so it plays on iOS), proxied/cached covers for every format — game box art, embedded ebook/audiobook art, and a rendered first page for magazines and PDF books (which carry no embedded cover), comic page count + per-page extraction (each comic page pulled from its CBZ/CBR/CB7 archive on the server, downscaled to a cached WebP), server-side game saves (the in-game battery save / "Continue" AND explicit save states) + reading + listening position + pinned folders (all roam across devices), and the unified resume surface (a radiant spotlight + "Jump back in" shelf) that resumes books/comics/documents (to your spot), audiobooks (chapter + position), and games (boot + in-game Continue).'],
  ['/api/cards/sets · /sets/{id} · /sets/{id}/wantlist · /wantlist · /search · /card/{id} · /image · /stats · /ownership/import · /ownership · /sync-status', 'The Pokémon TCG collection: browse every set (with a completion bar) and its cards overlaid with what you own, search all ~20k cards by name, and open a card for its metadata + market price + your copies. The catalog is a background-indexed local clone of the public pokemon-tcg-data dataset; card faces are proxied from images.pokemontcg.io and cached as WebP on demand (no bulk download). Ownership is imported once from a Pokéllector export (CSV/JSON keyed on set + card number) and then managed in HQ — tap a card to mark it owned / set a quantity / wishlist it (an import replaces the imported rows but preserves those in-HQ edits). With a free pokemontcg.io API key, market value is refreshed for owned cards only, daily, so collection value stays ≤1 day fresh. The "Buy missing" / "Shopping list" buttons turn the cards you lack into a TCGplayer Mass Entry want-list to paste + cart-optimize (fewest sellers = less shipping) — HQ builds the list, TCGplayer matches the sellers, so there’s no scraping.'],
]
