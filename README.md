...
| **Solar** | Live **Enphase** solar — current production, and on metered systems whole-home consumption plus net grid flow (exporting/importing), with today / last-7-days / lifetime energy. Read straight from the Envoy's local API via `pyenphase` (the library HA's own integration uses), so it handles the firmware-7+ token auth and auto-refresh; the module hides itself until an Envoy is configured. |
