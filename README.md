...
| Module | What you get |
|---|---|
| **Dashboard** | At-a-glance widgets: CPU/RAM/OS-disk/uptime (plus GPU load/VRAM/encode sessions when an NVIDIA card is present), storage usage, RAID health, per-drive SMART, running containers, Plex now-playing + recently-added. Key cards are **back-lit by live state** — the System card glows by health (emerald → amber → rose), and Printer / Plex / Tailscale light up while a job is printing / a stream is live / the tailnet is connected. |
