...
| `PAPERS_DIR` / `BOOKS_DIR` / `COMICS_DIR` | Folders for the Library's reading sections under `RAID_MOUNT` (optional; each section hides if unset). `PAPERS_DIR` = PDFs (magazines/papers); `BOOKS_DIR` = ebooks (EPUB/MOBI/AZW3 + PDFs); `COMICS_DIR` = comics (CBZ/CBR/CB7). |
