...
| Module | What you get |
|---|---|
| **Dashboard** | At-a-glance widgets: CPU/RAM/OS-disk/uptime (plus GPU load/VRAM/encode sessions when an NVIDIA card is present), storage usage, RAID health, per-drive SMART, running containers, Plex now-playing + recently-added. |
...
| **Library** | A mobile-first hub for content you **own and consume directly** — the counterpart to Plex's streamed video. Play your game ROMs in the browser (Game Boy / Color / GBA, NES, Super Nintendo, and Sega Genesis / Master System / Game Gear — via a self-hosted, pinned EmulatorJS engine, all client-side so the server stays a file server) straight from your storage, with **auto-fetched box art** (matched by name from libretro-thumbnails, downscaled to cached WebP thumbnails), cleaned-up titles, a per-game **detail page**, a **Recently Played** row, and **game saves that roam across your devices** (synced server-side and included in the off-site backup): your **in-game save** — the game's own "Save" → "Continue", captured as you play and seeded back when you reopen, so you resume on any device and offline — plus explicit **save states** (machine snapshots with screenshot thumbnails and one-tap Resume). Also read your own **ebooks** (EPUB / MOBI / AZW3 via a built-in foliate-js reader — no server-side conversion), **comics** (CBZ / CBR / CB7 — the backend extracts + downscales each page, so RAR/7z archives just work and big scans load fast; browsed by nested folder + search, with **pinnable folders** for one-tap return to a series, read with a page-by-page pager), and **magazines & papers** (PDFs via PDF.js — also browsed by **nested folder + search**, so dropping a series into a subfolder collapses it behind one row instead of flooding the list) in the browser, with **server-side reading position** that roams across devices. Plus **audiobooks** — a book is a folder of chapter files, played by an in-app audio player (chapter auto-advance, lock-screen / Control-Center transport via the Media Session API, background playback) that **resumes where you left off**, server-side, across devices — with **cover art** pulled from a folder image or the chapter's embedded tags. A big ebook library is **searchable by title or author** — a background indexer reads each book's embedded metadata into a small text-only cache, so even 10k+ books are a search box away (names normalized for display; files untouched), each result showing its **cover** — extracted from the book file on first view and cached as a WebP thumbnail, so only books you open ever take cache space. A **Jump back in** shelf on the Library hub resumes in-progress content across types — books and documents to your spot, games by booting to your in-game Continue. **Everything is downloadable for offline use** — tap a reader/player's **⬇** and the item (a PDF/ebook, every page of a comic, every chapter of an audiobook, or a ROM + its emulator core) is saved to the device by a service worker and reads/plays with no connection, on a plane; a **Downloads** page with **audit-grade accounting** shows exactly what's on the device with a *Verify storage* check, browse rows get a **✓ offline** badge, and the app falls back to your downloads when the server is unreachable. Your **place is saved even while offline** — position writes queue locally and sync back the moment the server is reachable again (last-write-wins), and a reader reopened offline resumes from that local spot — so reading on a plane still lands you in the right place on your next device. Built on a generic section framework + a read-only, range-capable, traversal-guarded file streamer, so a new content type is one section entry. Hides sections you haven't configured. |
...
| **Solar** | Live **Enphase** solar with an energy-flow visual: a glowing radial production **gauge** beside an animated **Sun → Home ↔ Grid** flow (exporting/importing), warm gradient energy tiles (today / last-7-days / lifetime), a produced-vs-used balance on metered systems, and an intraday **day curve** from a built-in trend sampler. Read straight from the Envoy's local API via `pyenphase` (the library HA's own integration uses), so it handles the firmware-7+ token auth and auto-refresh; the module hides itself until an Envoy is configured. |
| **Weather** | Current conditions (temp, feels-like, humidity, wind, with day/night condition icons) + a **5-day forecast** as color-coded daily rows (lo→hi range bars) where **tapping a day expands its hourly breakdown** (temp + precipitation). A current-conditions **hero banner** also leads the Dashboard and links here. From **Open-Meteo** — free, no API key. Hides itself until a location is configured. |
...
| `PAPERS_DIR` / `BOOKS_DIR` / `COMICS_DIR` / `AUDIOBOOKS_DIR` | Folders for the Library's content sections under `RAID_MOUNT` (optional; each section hides if unset). `PAPERS_DIR` = PDFs (magazines/papers); `BOOKS_DIR` = ebooks (EPUB/MOBI/AZW3 + PDFs); `COMICS_DIR` = comics (CBZ/CBR/CB7); `AUDIOBOOKS_DIR` = audiobooks (folders of audio files). |
