...
| **Library** | A mobile-first hub for the content you **own and consume directly** — the counterpart to Plex's streamed video. Play your **game ROMs** in the browser (Game Boy / GBA, NES, SNES, Genesis — a self-hosted, all-client-side EmulatorJS) with auto-fetched box art and **saves that roam across devices** — browse **one system at a time** (a box-art card per console → that console's games alphabetically, with an **A→Z thumb scrubber** and a per-system search to find a title fast — your place is kept in the URL, so opening a game and tapping Back returns you mid-search, not to a blank list); read your own **ebooks** (EPUB / MOBI / AZW3), **textbooks & reference books** (the same formats, organized into browsable sub-categories like Programming / Cooking / Game Design), **comics** (CBZ / CBR / CB7), and **magazines & papers** (PDF), and listen to **audiobooks** — each with **server-side reading/listening position** that follows you device to device, and cover art on every item. The **hub leads with your content** — the most-recent in-progress item becomes a radiant spotlight (one tap to resume), with cover-art shelves beneath. And **everything is downloadable for offline use**: a service worker caches a book, comic, audiobook, or ROM so it reads/plays with no connection, and your place still syncs back once you're online again. Each section hides until you point it at a folder. |
| **Cards** | A self-hosted **Pokémon TCG collection** browser — the counterpart to the Library, for a collection that isn't files on disk. Browse **every set** (each with a completion bar), see its cards with the ones you **own in full colour and the rest dimmed**, **search all ~20k cards** by name, and open any card for its details, market value, and your copies. The catalog is a background-indexed local clone of the public `pokemon-tcg-data` dataset, and card faces are proxied + cached as WebP **on demand** (no 20 GB download). A back-lit **stats hero + show-off wall** puts your collection front and centre. Your ownership is **imported once** from a Pokéllector export (CSV/JSON) and then **managed in HQ** — tap any card to **mark it owned, set a quantity, or add it to your wishlist**; a re-import refreshes the imported cards but keeps your in-HQ edits. With a free pokemontcg.io API key, **collection value** is refreshed for your owned cards daily (≤1 day fresh). A **"Buy missing"** button turns the cards you lack into a **TCGplayer Mass Entry** want-list to paste and cart-optimize (fewest sellers → less shipping). Hides until you point it at a dataset clone. |
...
| `CARD_DATA_SRC` | For the **Cards** module: a local clone of the public [`pokemon-tcg-data`](https://github.com/PokemonTCG/pokemon-tcg-data) dataset (`git clone` it once; mounted read-only). Optional — defaults to a tiny committed example. Optional extras: `POKEMONTCG_API_KEY` (free key from dev.pokemontcg.io → shows market value), `CARDS_DIR` (a local full-res image dump for fully-offline faces). |
