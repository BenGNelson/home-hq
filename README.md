...
| **Solar** | Live **Enphase** solar with an energy-flow visual: a glowing radial production **gauge** beside an animated **4-node Solar · Battery · Grid · Home** flow. On systems with **IQ batteries** it shows state-of-charge (with the backup-reserve marker), usable energy, live charge/discharge, and a **self-sufficiency %**; plus warm energy tiles (today / 7-day / lifetime), a produced-vs-used balance, intraday production + battery-charge **curves**, and a **per-panel array** map (each microinverter shaded by output). Read straight from the Envoy's local API via `pyenphase` (the library HA's own integration uses), so it handles the firmware-7+ token auth and auto-refresh; the module hides itself until an Envoy is configured. |
