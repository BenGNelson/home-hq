...
| **Cameras** | A live camera wall fed by a read-only **Home Assistant** bridge — each tile relays HA's MJPEG through the backend (the browser never holds the HA token), connecting on demand so cameras sleep when nobody's watching. Hides itself until HA is configured. |
