...
def test_logs_endpoint_blocks_excluded_container_requested_by_id(client, monkeypatch):
    ...
    monkeypatch.setattr(settings, "container_logs_exclude", "vpn-gateway")

    class _Excluded:
        name = "/vpn-gateway"  # Docker reports the name with a leading slash
    ...
    # check passes) — the resolved name is still vpn-gateway, so the re-check withholds it.
    body = client.get("/api/containers/" + "a" * 64 + "/logs").json()
    assert body["available"] is False and body["excluded"] is True
    assert body["name"] == "vpn-gateway"  # leading slash stripped
