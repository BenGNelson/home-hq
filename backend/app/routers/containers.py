...
EXCLUDE lists containers whose logs are withheld (e.g. a VPN gateway or an
auth proxy — the most sensitive, and the ones you'd debug over SSH instead).
...
def get_container_logs(name: str, tail: int = _LOGS_TAIL_DEFAULT):
    ...
    an auth proxy. Reachable only over the LAN/tailnet, never the public internet.
    """
