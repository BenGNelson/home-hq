...
services (UFW limits Home Assistant, downloader, etc. to the LAN subnet, and the
...
activity like transfer names), so it's only sound because the UI is reachable
...
transfer client — the most sensitive and the ones you'd `docker logs` over SSH
